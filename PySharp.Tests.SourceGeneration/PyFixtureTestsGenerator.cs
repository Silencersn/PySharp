...
public sealed class PyFixtureTestsGenerator : IIncrementalGenerator
{
    ...
    private static void Emit(SourceProductionContext context, ImmutableArray<FixtureInfo> fixtures)
    {
        ...
        context.AddSource("PyFileTests.g.cs", SourceText.From(
            EmitTestClass(TestClassName, RunnerType, tests, ignoreReason: null), Encoding.UTF8));
        ...
        context.AddSource("PyFileCpythonTests.g.cs", SourceText.From(
            EmitTestClass(DiffTestClassName, DiffRunnerType, tests, static f => f.CpythonDiffReason), Encoding.UTF8));
    }

    private static string EmitTestClass(string className, string runnerType, FixtureInfo[] tests, Func<FixtureInfo, string?>? ignoreReason)
    {
