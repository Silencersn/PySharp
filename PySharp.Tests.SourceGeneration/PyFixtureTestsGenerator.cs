...
/// declares :kind: test, plus a matching CPython output-comparison test class
/// driven by PyCpythonDiffRunner (fixtures with a :cpython-diff: reason are
/// [Ignore]d there). Fixtures marked :kind: helper are imported by other
...
/// "regression" fails the build; unknown fields, "Regression:" title lines
/// and an empty :cpython-diff: reason warn.
/// </summary>
[Generator]
public sealed class PyFixtureTestsGenerator : IIncrementalGenerator
{
    ...
    private const string DiffRunnerType = "PyCpythonDiffRunner";
    private const string DiffTestClassName = "PyFileCpythonTests";
    private const string TestNamespace = "PySharp.Tests";

    // Per-test wall-clock caps, emitted as [Timeout] so a fixture that blocks
    // (e.g. on ambient stdin) is reported as a failure instead of stalling the
    // whole run. The diff runner spends up to 60s per side, so it needs more.
    private const int InProcessTimeoutMs = 60_000;
    private const int DiffTimeoutMs = 180_000;
    ...
    private static readonly DiagnosticDescriptor EmptyCpythonDiffReason = new(
        id: "PYFIX009",
        title: "Empty ':cpython-diff:' reason",
        messageFormat: "Fixture '{0}': ':cpython-diff:' requires a reason describing the known CPython divergence",
        category: "PySharp.Tests.SourceGeneration",
        defaultSeverity: DiagnosticSeverity.Warning,
        isEnabledByDefault: true);
    ...
    private static void Emit(SourceProductionContext context, ImmutableArray<FixtureInfo> fixtures)
    {
        ...
        context.AddSource("PyFileTests.g.cs", SourceText.From(
            EmitTestClass(TestClassName, RunnerType, tests, ignoreReason: null, timeoutMs: InProcessTimeoutMs), Encoding.UTF8));
        // The comparison class runs the same fixtures as scripts under a
        // local CPython 3.14 and asserts both runs agree; fixtures with a
        // registered known divergence (:cpython-diff: reason) are emitted
        // with [Ignore] until the divergence is fixed.
        context.AddSource("PyFileCpythonTests.g.cs", SourceText.From(
            EmitTestClass(DiffTestClassName, DiffRunnerType, tests, static f => f.CpythonDiffReason, timeoutMs: DiffTimeoutMs), Encoding.UTF8));
    }

    private static string EmitTestClass(string className, string runnerType, FixtureInfo[] tests, Func<FixtureInfo, string?>? ignoreReason, int timeoutMs)
    {
        ...
        {
            builder.Append("/// One CPython output-comparison test per Python fixture in test_pyfiles,").Append('\n');
            builder.Append("/// generated by PyFixtureTestsGenerator from the fixture docstring metadata.").Append('\n');
            builder.Append("/// Each test runs the fixture under PySharp.Console and a local CPython 3.14").Append('\n');
            builder.Append("/// and asserts both runs agree (see PyCpythonDiffRunner).").Append('\n');
        }
        builder.Append("/// </summary>").Append('\n');
        builder.Append("[TestClass]").Append('\n');
        builder.Append("public sealed class ").Append(className).Append('\n');
    ...
    private sealed class FixtureInfo : IEquatable<FixtureInfo>
    {
        ...
        private FixtureInfo(string fileName, string? methodName, FixtureKind kind, string? cpythonDiffReason, ImmutableArray<(DiagnosticDescriptor Descriptor, string[] MessageArgs)> diagnostics)
        {
            FileName = fileName;
            MethodName = methodName;
            Kind = kind;
            CpythonDiffReason = cpythonDiffReason;
        ...
        /// <summary>Reason registered via :cpython-diff:; non-null marks a known divergence.</summary>
        public string? CpythonDiffReason { get; }
        ...
        public bool Equals(FixtureInfo? other) =>
            ...
            && Kind == other.Kind
            && CpythonDiffReason == other.CpythonDiffReason
        ...
        public static FixtureInfo Parse(string path, string text)
        {
            ...
            var cpythonDiffReason = default(string?);
            ...
            foreach (Match field in FieldRegex.Matches(docstring))
            {
                ...
                switch (name)
                {
                    ...
                    case "background":
                        ...
                        break;
                    case "cpython-diff":
                        // Registers a known divergence from CPython for the
