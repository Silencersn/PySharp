...
// XID_Start / XID_Continue code point ranges (inclusive pairs, non-ASCII
// only) as defined by UAX #31; ASCII is covered by the fast paths below.
internal static class UnicodeIdentifier
{
    private static readonly int[] XidStartRanges =
    [
    ...
    private static readonly int[] XidContinueRanges =
    [
    ...
    public static bool IsXidStart(int codePoint)
        => codePoint < 0x80
            ? char.IsAsciiLetter((char)codePoint) || codePoint is 0x5F
            : IsInRanges(XidStartRanges, codePoint);

    public static bool IsXidContinue(int codePoint)
        => codePoint < 0x80
            ? char.IsAsciiLetterOrDigit((char)codePoint) || codePoint is 0x5F
            : IsInRanges(XidContinueRanges, codePoint);

    private static bool IsInRanges(int[] ranges, int codePoint)
    {
        var low = 0;
